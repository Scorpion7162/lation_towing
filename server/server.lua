...
local activeTowTrucks = {}
local activeJobVehicles = {}
local towedVehicles = {}
local activeTowDrivers = {}
local jobPeds = {}
...
local function VerifyVehicleAccess(src, netId)
    ...
    if activeTowTrucks[netId] and activeTowTrucks[netId].owner == src then
        return true
    end
    if activeJobVehicles[netId] and (activeJobVehicles[netId].owner == src or src == activeJobVehicles[netId].tower) then
        return true
    end
    if towedVehicles[netId] and towedVehicles[netId].tower == src then
        return true
    end
...
lib.callback.register('lation_towtruck:spawnTowTruck', function(source)
    local src = source
    if activeTowDrivers[src] and activeTowDrivers[src].towTruckNetId then
        local vehicle = NetworkGetEntityFromNetworkId(activeTowDrivers[src].towTruckNetId)
    ...
    activeTowTrucks[netId] = {
        owner = src,
        timestamp = os.time()
    }
  if activeTowDrivers[src] then
        activeTowDrivers[src].towTruckNetId = netId
    end
...
lib.callback.register('lation_towtruck:canTowVehicle', function(source, towTruckNetId, vehicleNetId)
    ...
    if not activeTowTrucks[towTruckNetId] or activeTowTrucks[towTruckNetId].owner ~= src then
    ...
    for _, data in pairs(towedVehicles) do
...
lib.callback.register('lation_towtruck:canDetachVehicle', function(source, vehicleNetId)
    ...
    if not towedVehicles[vehicleNetId] or towedVehicles[vehicleNetId].tower ~= src then
...
lib.callback.register('lation_towtruck:checkTowTruck', function(source, netId)
    return activeTowTrucks[netId] ~= nil
end)
lib.callback.register('lation_towtruck:clockIn', function(source)
    local src = source
    if activeTowDrivers[src] and activeTowDrivers[src].onDuty then
        LogSecurityEvent(src, 'ClockIn', 'Already clocked in')
        return false
    end
    activeTowDrivers[src] = activeTowDrivers[src] or {}
    activeTowDrivers[src].onDuty = true
    activeTowDrivers[src].timestamp = os.time()
    activeTowDrivers[src].lastJob = 0
    return true
end)
lib.callback.register('lation_towtruck:clockOut', function(source)
    local src = source
    if not activeTowDrivers[src] or not activeTowDrivers[src].onDuty then
        LogSecurityEvent(src, 'ClockOut', 'Not clocked in')
        return false
    end
    activeTowDrivers[src].onDuty = false
    for netId, data in pairs(activeJobVehicles) do
...
lib.callback.register('lation_towtruck:cleanupDelivery', function(source, vehicleNetId)
    ...
    local vehicle = NetworkGetEntityFromNetworkId(vehicleNetId)
    if DoesEntityExist(vehicle) then
        for towTruckNetId, data in pairs(activeTowTrucks) do
            local towTruck = NetworkGetEntityFromNetworkId(towTruckNetId)
            if DoesEntityExist(towTruck) then
                DetachEntity(vehicle, true, true)
            end
        end
        towedVehicles[vehicleNetId] = nil
        activeJobVehicles[vehicleNetId] = nil
...
RegisterNetEvent('lation_towtruck:startTowing', function(towTruckNetId, vehicleNetId)
    ...
    for existingNetId, data in pairs(towedVehicles) do
...
RegisterNetEvent('lation_towtruck:stopTowing', function(vehicleNetId)
    ...
    TriggerClientEvent('lation_towtruck:vehicleTowState', -1, vehicleNetId, 0, false)
    if activeJobVehicles[vehicleNetId] then
        activeJobVehicles[vehicleNetId].tower = nil
    end
    towedVehicles[vehicleNetId] = nil
    if Config.EnableServerLogs then
...
RegisterNetEvent('lation_towtruck:requestJob', function(isEmergency)
    ...
    local lastJobTime = activeTowDrivers[src].lastJob or 0
    ...
    activeTowDrivers[src].lastJob = currentTime
...
RegisterNetEvent('lation_towtruck:deleteTowTruck', function(netId)
    local src = source
    if not activeTowTrucks[netId] or activeTowTrucks[netId].owner ~= src then
    ...
    activeTowTrucks[netId] = nil
    if activeTowDrivers[src] then
        activeTowDrivers[src].towTruckNetId = nil
    end
    if Config.EnableServerLogs then
...
AddEventHandler('onResourceStop', function(resource)
    ...
    for netId, data in pairs(activeTowTrucks) do
        local vehicle = NetworkGetEntityFromNetworkId(netId)
        if DoesEntityExist(vehicle) then
            DeleteEntity(vehicle)
        end
    end
    for _, ped in ipairs(jobPeds) do
...
AddEventHandler('playerDropped', function()
    ...
    for netId, data in pairs(towedVehicles) do
        if data.tower == src then
            local vehicle = NetworkGetEntityFromNetworkId(netId)
            if DoesEntityExist(vehicle) then
                DetachEntity(vehicle, true, true)
                TriggerClientEvent('lation_towtruck:vehicleTowState', -1, netId, 0, false)
                if activeJobVehicles[netId] then
                    DeleteEntity(vehicle)
                    activeJobVehicles[netId] = nil
                end
            end
            towedVehicles[netId] = nil
        end
    end
end)
    ...
    while true do
        ...
        for netId, data in pairs(activeJobVehicles) do
            if currentTime - data.timestamp > Config.JobVehicleTimeout then
                local vehicle = NetworkGetEntityFromNetworkId(netId)
                if DoesEntityExist(vehicle) then
                    if towedVehicles[netId] then
                        local towTruckNetId = towedVehicles[netId].towTruckNetId
                        local towTruck = NetworkGetEntityFromNetworkId(towTruckNetId)
                        if DoesEntityExist(towTruck) then
                            DetachEntity(vehicle, true, true)
                        end
